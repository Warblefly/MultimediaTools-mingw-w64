...
#	git clone --depth 1 --single-branch -b binutils-2_44-branch https://github.com/bminor/binutils-gdb.git binutils || echo "Seems we have binutils."
	git clone --depth 1 --single-branch -b binutils-2_46-branch git://sourceware.org/git/binutils-gdb.git binutils || echo "Seems we have binutils."
echo "Binutils has arrived."
...
git clone --branch v14.x git://git.code.sf.net/p/mingw-w64/mingw-w64 mingw-w64 || echo "Seems we have mingw-w64."
...
git clone --depth 1 --single-branch -b releases/gcc-15 https://github.com/gcc-mirror/gcc.git gcc || echo "Seems we have GCC."
...
cd gcc
	...
	if [[ ! -f gcc_accessories_source ]]; then
		...
		wget https://ftp.gnu.org/gnu/mpc/mpc-1.4.1.tar.xz || exit 1
		tar xvvf mpc-1.4.1.tar.xz && ln -sv mpc-1.4.1 mpc
